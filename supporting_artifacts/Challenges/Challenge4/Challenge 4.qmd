...
```{r}
homes_clean <- homes_clean |> 
  mutate(numbers = 1:nrow(homes_clean))

avos_clean <- avos_clean |> 
  mutate(numbers = 1:nrow(avos_clean))

homes_avos <- homes_clean |> 
  full_join(avos_clean, by = "city") |> 
  arrange(home_price) |> 
  distinct(`Total Volume`, .keep_all = TRUE)

ggplot(data = homes_avos, mapping = aes(x = Total_Volume))
```

1.  Then I joining the data set by the 4 city regions [with averages]{.underline} (means) of total volume sold of avocados, average price of avocados, and average price of homes. This worked well!
